...
class SalesView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        start_date = timezone.now().today().date() - datetime.timedelta(hours=24)
        end_date = timezone.now().today().date() + datetime.timedelta(hours=12)


        today_data       = qs.by_today().get_sales_breakdown()
        week_data        =  qs.by_weeks_range(weeks_ago=1, number_of_weeks=1).get_sales_breakdown()
        last_four_weeks  =  qs.by_weeks_range(weeks_ago=4, number_of_weeks=4).get_sales_breakdown()
        print("1111111",last_four_weeks)
        context['today']           = today_data
        context['this_week']       = week_data
        context['last_four_weeks'] = last_four_weeks
        return context